...
`endif
    ...
    always @(posedge clk) begin
        if ((rst == `RstEnable) || (uart_debug_pin == 1'b1)) begin
            ...
            $display("[SID_TB] UART_TXDATA write[%0d] = 0x%02x (%c), pc=0x%08x",
    ...
    always @(posedge clk) begin
        if ((rst == `RstDisable) && (uart_debug_pin == 1'b0) &&
            ...
            $display("[SID_TB] RA_WRITE: pc=0x%08x inst_ex=0x%08x ra<=0x%08x ctrl_state=0x%0x",
                     u_dut.u_tinyriscv_soc_top.u_tinyriscv.pc_pc_o,
    ...
    always @(posedge clk) begin
        if ((rst == `RstDisable) && (uart_debug_pin == 1'b0) &&
            ...
            $display("[SID_TB] SP_WRITE: pc=0x%08x inst_ex=0x%08x sp<=0x%08x ctrl_state=0x%0x",
                     u_dut.u_tinyriscv_soc_top.u_tinyriscv.pc_pc_o,
                     u_dut.u_tinyriscv_soc_top.u_tinyriscv.ie_inst_o,
                     u_dut.u_tinyriscv_soc_top.u_tinyriscv.ex_reg_wdata_o,
                     u_dut.u_tinyriscv_soc_top.u_tinyriscv.ctrl_state_o);
    ...
    always @(posedge clk) begin
        if ((rst == `RstDisable) && (uart_debug_pin == 1'b0) &&
            ...
            $display("[SID_TB] LOAD_REQ: pc=0x%08x inst_ex=0x%08x addr=0x%08x data_i=0x%08x ack=%b ctrl_state=0x%0x",
                     u_dut.u_tinyriscv_soc_top.u_tinyriscv.pc_pc_o,
                     u_dut.u_tinyriscv_soc_top.u_tinyriscv.ie_inst_o,
    ...
    always @(posedge clk) begin
        if ((rst == `RstDisable) && (uart_debug_pin == 1'b0) &&
            ...
            $display("[SID_TB] STORE_REQ: pc=0x%08x inst_ex=0x%08x addr=0x%08x data=0x%08x ack=%b ctrl_state=0x%0x",
                     u_dut.u_tinyriscv_soc_top.u_tinyriscv.pc_pc_o,
                     u_dut.u_tinyriscv_soc_top.u_tinyriscv.ie_inst_o,
                     u_dut.u_tinyriscv_soc_top.u_tinyriscv.rib_ex_addr_o,
    ...
    always @(posedge clk) begin
        if (u_dut.u_bridge_slave_top.u_ram.we_i == `WriteEnable) begin
            $display("[SID_TB] RAM_WRITE: addr=0x%08x word_index=0x%08x data=0x%08x",
    ...
    initial begin
        inst_file = "liudk_test/sID_test/sID_inst.data";
        rom_dump_file = "liudk_test/sID_test/build/downloaded_rom_after_uart.hex";
...
`ifndef NO_DUMP
`ifdef FSDB
        $fsdbDumpfile("liudk_test/sID_test/build/sID_tb.fsdb");
        $fsdbDumpvars(0, sID_tb);
        $fsdbDumpMDA(0, sID_tb);
`else
        $dumpfile("liudk_test/sID_test/build/sID_tb.vcd");
        $dumpvars(0, sID_tb);
`endif
`endif
    end
...
`endif
        $display("[SID_TB] TIMEOUT: x26=0x%08x x27=0x%08x pc=0x%08x succ=%b",
                 ...
                 u_dut.u_tinyriscv_soc_top.u_tinyriscv.pc_pc_o,
                 succ);
        $display("[SID_TB] LAST_VALID: pc=0x%08x inst_ex=0x%08x ctrl_state=0x%0x",
    ...
    task dump_ram_contents;
        integer ram_i;
        begin
            $display("[SID_TB] RAM_DUMP_BEGIN");
            for (ram_i = 0; ram_i < `MemNum; ram_i = ram_i + 1) begin
                $display("[SID_TB] RAM[%0d] = 0x%08x", ram_i, u_dut.u_bridge_slave_top.u_ram._ram[ram_i]);
            end
            $display("[SID_TB] RAM_DUMP_END");
        end
    endtask
